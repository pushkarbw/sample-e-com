...
describe('9ABF Cart and Checkout Business Logic Failures', () => {
  ...
  beforeEach(() => {
    ...
    cy.window().then((win) => {
      Object.defineProperty(win.navigator, 'userAgent', {
        writable: true,
        value: 'Mozilla/5.0 9ABF Test Agent'
      });
    });
  ...
  it('9ABF should calculate multi-item discount correctly', () => {
    cy.visit('/products');
    cy.get('[data-testid="products-container"], .products-container').should('be.visible');
    cy.wait(2000);
  ...
  it('9ABF should preserve product state after failed cart operations', () => {
    ...
    cy.get('body').then(($body) => {
      ...
      if (addToCartButtons.length > 0) {
        ...
        cy.get('[data-testid="cart-item"], .cart-item').then($initialItems => {
          ...
          cy.get('body').then(($cartBody) => {
            ...
            if (removeButtons.length > 0) {
              ...
              cy.get('[data-testid="cart-item"], .cart-item').should('have.length', initialCount, 
                'Cart item should remain when backend removal fails - inconsistent state bug');
            }
          });
        });
      }
